...
class DataParser:
    ...
    def __form_attachments_files_list_from_file(self, file_name, file_content_to_parse):
    ...
    def __return_api_info_from_log_file(self, log_file_content):
    ...
    def __return_api_info_from_fact_json_file(self, file_name, json_file_fact_content):
    ...
    def __return_api_info_from_expected_json_file(self, file_name, json_file_expected_content):
        PATTERN_UUID4 = r'[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}'
        PATTERN_OBJ_ID = r'\/[^v]?\d{1,5}\D?\/'
        api_methods_info = []
    ...
    def _set_data_frame_from_attach(self, file, file_content, data_set_to_save_data_frames):
        attachments = self.__form_attachments_files_list_from_file(
            file_name=file,
            file_content_to_parse=file_content
        )
        if attachments:
            self.logger.debug(f'Try to set_data_frame_from_attach from {file}')
        ...
        self.logger.debug(f'Parsing of {file} is complete')

    def _set_data_frame_from_json(self, file_name, file_content, data_set_to_save_data_frames):
